...
void    clear_tab(char **tab)
{
    ...
    if (tab)
    {
        ...
        {
            free(tab[i]);
            tab[i] = NULL;  // Dodane zerowanie
            i++;
        }
        free(tab);
        tab = NULL;  // Dodane zerowanie
    }
}
...
void    clear_env(t_data *env)
{
    ...
    if (env->env_var)
    {
        ...
        env->env_var = NULL;  // Dodane zerowanie
    }
    // Dodane sprawdzenie innych pól struktury env
    if (env->envp)
    {
        clear_tab(env->envp);
        env->envp = NULL;
...
void    exit_shell(t_data *env, char *mess, int fail)
{
    ...
    clear_env(env);  // Wyczyść env tylko raz
    if (fail == -1)
        exit(EXIT_FAILURE);
    else if (!fail)
        exit(EXIT_SUCCESS);
